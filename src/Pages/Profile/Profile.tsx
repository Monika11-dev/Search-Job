...
const Profile = () => {
  ...
  const validateForm = () => {
    ...
    if (!formValues.title) newErrors.title = "Title Name is required.";
    else if (!regexUsername.test(formValues.title))
      newErrors.title = "title can only contain letters and spaces.";

    if (!formValues.languages) newErrors.languages = "Language is required.";

    if (!formValues.current) newErrors.current = "Current salary is required.";
    ...
    if (!formValues.pincode) newErrors.pincode = "Zipcode is required.";
    ...
    if (!formValues.mobile) newErrors.mobile = "mobile is required.";
    else if (!/^\d{10}$/.test(formValues.mobile))
      newErrors.mobile = "Phone must be a 10-digit number.";

    if (!formValues.email) newErrors.email = "Email is required.";
    else if (
      !/^[\w.%+-]+@[A-Za-z0-9.-]+\.[A-Z|a-z]{2,}$/.test(formValues.email)
    )
    ...
    if (!formValues.street) newErrors.street = "Street Address is required.";
